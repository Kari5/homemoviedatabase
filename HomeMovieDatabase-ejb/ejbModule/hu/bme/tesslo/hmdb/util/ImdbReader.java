...
public class ImdbReader {
	...
	public ImdbReader(String movieName, Integer movieYear, String imdbID) {
		...
		logger.info("Keresendõ film: "+this.movieName+", "+this.movieYear+", "+this.imdbID);
		if (this.imdbID != null &&  !this.imdbID.isEmpty() && !this.imdbID.equals("null") && !this.imdbID.equals(" ")) {
	...
	/**
	 * Az összeállított url-t meghívja, és elmenti egy string-be a visszakapott adatokat.
	...
	private String readRawData() {
		...
		try {
			...
			logger.info(get.getName()+", "+get.getPath()+", "+get.getURI());
	...
	public Movie parseJson(){
		...
		try {
			...
			result=new Movie(title, year, runtime, genere, director, writer, actors, plot, posterUrl, imdbRating, imdbId, null, null, null);
